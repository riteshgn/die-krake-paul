...
class KaggleExtractor(object):
    ...
    def prepare_dataset(self, season, league_id):
        ...
        df_match_results = df_matches[['id', 'home_team', 'away_team', 'match_date', 'result']]
        ...
        filename = 'kaggle_db/features_{}_{}_v2_all.csv'.format(season.replace('/', '_'), league_id)
        df_matchday_player_stats.to_csv(filename)
        print('Extraction complete... [(matches, feature_count): {shape} saved to file: {filename}]'.format(
            records=df_matchday_player_stats.size, shape=df_matchday_player_stats.shape, filename=filename))

    def _collect_home_player_stats(self, season, league_id):
        query = \
            'select ' \
            '   m.id, m.date as ps_date, ' \
    ...
    def _collect_match_stats(self, season, league_id):
        query = 'select ' \
                '   m.id, c.name as country, l.name as league, m.stage as gameweek, m.date as match_date, ' \
        ...
        df_matches = df_matches.apply(_enhance_match_results, axis=1)
        return df_matches
...
def _append_player_ages(player, player_type, df_player_avg_overall_ratings):
    for plyr_indx in range(1, 12):
        ...
        player[key_plyr_age] = (player['ps_date'] - player[key_plyr_bday]).days / 365
...
def _enhance_player_stats(df, player_type, df_player_avg_overall_ratings):
    df['ps_date'] = df['ps_date'].apply(pd.to_datetime)
    for plyr_indx in range(1, 12):
...
def _enhance_match_results(match):
    return _set_result(match)
